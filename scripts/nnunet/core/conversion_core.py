...
def _build_dataset_json_payload(
    ...
    return {
        ...
        "file_ending": ".nii.gz",
        "description": "ISLES24 2D slices preprocessed with MedSegDiff pipeline",
...
def run_conversion(cfg: DictConfig) -> Dict[str, Any]:
    """
    Run nnU-Net conversion for online-slice datasets using the slice strategy.
    """
    setup_seeds(cfg)
    ...
    print(f"{'=' * 60}")
    print(f"Mode: {'TEST (limited slices)' if request.is_test_mode else 'FULL EXPORT'}")
    if request.is_test_mode:
        print(f"Max slices per split: {request.max_slices}")
    print(
        "Export splits: "
    ...
    if request.is_test_mode:
        if request.export_train:
            print(f"Will export train: {min(request.max_slices, len(train_dataset))} slices")
        if request.export_test:
            print(f"Will export test: {min(request.max_slices, len(val_dataset))} slices")
    print()

    strategy = SliceExportStrategy()
    train_case_ids = set()
    val_case_ids = set()
    ...
    if request.is_test_mode:
        print(f"\n⚠️  TEST MODE: Only exported {request.max_slices} slices per split")
    ...
    print(
        ...
        f"-o {pred_dir} -d {dataset_id} -c 2d -f all"
    )
    ...
    print(f"       -d {dataset_id} -i {request.images_ts} -o {pred_dir}")
    print(
