...
# Stub for batchgenerators - not needed for training/sampling pipeline
def pad_nd_image(*args, **kwargs):
    """
    Stub for batchgenerators.pad_nd_image.
    
    Only used in SegmentationNetwork sliding window inference methods.
    For future use, consider MONAI's SlidingWindowInferer.
    """
    raise NotImplementedError(
        "pad_nd_image requires batchgenerators package. "
        "Consider using MONAI's SlidingWindowInferer for sliding window inference."
    )
...
# Import nn utilities from improved_diffusion (shared with OpenAI diffusion code)
from src.improved_diffusion.nn import (
    checkpoint,
    conv_nd,
    linear,
    avg_pool_nd,
