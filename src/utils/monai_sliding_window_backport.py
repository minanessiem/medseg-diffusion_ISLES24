...
"""
Backported MONAI sliding window inferer utilities.

This module vendors MONAI's sliding-window implementation with tuple-index fixes
for PyTorch >= 2.9 compatibility. It is used to avoid upgrading MONAI in-place
while preserving upstream inference semantics.
"""
...
def sliding_window_inference(
    ...
    **kwargs: Any,
    ...
    # Only perform strict shape validation if roi_size is a sequence.
...
def _create_buffered_slices(slices, batch_size, sw_batch_size, buffer_dim, buffer_steps):
    ...
    b_ends = np.cumsum(_b_lens).tolist()
    ...
    for _s, _r in enumerate(windows_range):
        ...
        b_slices.append((_r.stop, s_s, s_e))
